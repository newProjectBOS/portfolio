        ...
        >
            <div className="flex flex-col p-7 gap-5">
                <div>
                    ...
                    <p className={["text-[13px] font-pliant", offer.dark ? "text-zinc-500" : "text-zinc-400"].join(" ")}>
                        {offer.description}
                    </p>
                </div>
                <div className="flex items-baseline gap-1.5">
                    <span className={["text-5xl font-pliant font-normal leading-none tracking-tight", offer.dark ? "text-white" : "text-black"].join(" ")}>
                        {offer.price}
                    </span>
                ...
                <ul className="flex flex-col gap-2">
                    {offer.features.map((f: string, i: number) => (
                        <li key={i} className={["text-[13px] font-pliant", offer.dark ? "text-zinc-400" : "text-zinc-500"].join(" ")}>
                            {f}
                        </li>
                    ))}
                </ul>
                <button
                    className={[
                        "w-full py-2.5 border rounded-md text-[10px] font-pliant tracking-[0.15em] uppercase transition-colors duration-150",
                        offer.dark
